    ...
    <div className="w-full flex flex-col gap-8">
      <div className="flex flex-col lg:flex-row gap-8 items-stretch">
        ...
        >
          ...
          <div className="opacity-0 md:group-hover:opacity-100 transition-opacity duration-300 absolute inset-y-0 left-0 w-1/2 flex items-center justify-start p-4">
            <button onClick={e => { e.stopPropagation(); setActive(p => (p - 1 + items.length) % items.length); }} className="w-16 h-16 rounded-full bg-black/40 hover:bg-brand backdrop-blur-md text-foreground dark:text-white flex items-center justify-center border border-white/10 transition-colors">
          ...
          <div className="opacity-0 md:group-hover:opacity-100 transition-opacity duration-300 absolute inset-y-0 right-0 w-1/2 flex items-center justify-end p-4">
            <button onClick={e => { e.stopPropagation(); setActive(p => (p + 1) % items.length); }} className="w-16 h-16 rounded-full bg-black/40 hover:bg-brand backdrop-blur-md text-foreground dark:text-white flex items-center justify-center border border-white/10 transition-colors">
      ...
      {lightboxOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 md:p-8">
          ...
          <div className="relative w-full max-w-6xl max-h-[90vh] flex flex-col bg-surface dark:bg-[#111111] border border-white/10 rounded-3xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300">
            <button onClick={() => setLightboxOpen(false)} className="absolute top-4 right-4 z-10 w-12 h-12 flex items-center justify-center rounded-full bg-black/50 hover:bg-brand text-foreground dark:text-white backdrop-blur-md transition-colors">
            ...
            <div className="p-6 bg-surface-strong dark:bg-[#0a0a0a] border-t border-white/10">
