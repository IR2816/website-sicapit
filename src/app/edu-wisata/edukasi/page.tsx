...
function ActivitiesSection() {
  const activities = [
    {
      image: '/edu-wisata.png',
      icon: Leaf,
      title: 'Edu Wisata & Pertanian',
  ...
  return (
    <AnimatedSection id="kategori" className="py-16 lg:py-20 bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <Tabs defaultValue="edu-wisata" className="w-full">
          <TabsList className="grid w-full grid-cols-3 mb-8 bg-white shadow-md border h-auto p-1">
            <TabsTrigger value="edu-wisata" className="py-3 sm:py-4 data-[state=active]:bg-emerald-600 data-[state=active]:text-white text-gray-600 text-xs sm:text-sm font-semibold rounded-lg transition-all">
              <Flower2 className="w-4 h-4 sm:w-5 sm:h-5 mr-1.5 sm:mr-2" />
              <span className="hidden xs:inline">Edu Wisata</span>
          ...
          {/* Edu Wisata Tab */}
          <TabsContent value="edu-wisata" className="mt-0">
            <div className="grid md:grid-cols-2 gap-5">
              {eduwisataItems.map((item, i) => (
                <motion.div
                  key={i}
...
function FAQSection() {
  const faqs = [
    ...
    },
    {
      ...
      a: 'Indonesia memiliki banyak kampung wisata yang telah diakui secara nasional dan internasional. Beberapa yang terkenal antara lain: Desa Penglipuran di Bali yang terkenal dengan arsitektur rumah adatnya yang seragam, Desa Borobudur di Jawa Tengah yang menawarkan wisata budaya di sekitar Candi Borobudur, Desa Wae Rebo di Flores yang terkenal dengan rumah adatnya yang unik, Desa Sambi di Boyolali yang terkenal dengan edu-wisata kopi dan sayuran, serta Desa Nglanggeran di Gunungkidul yang terkenal dengan geo-wisata dan sunrise point-nya.'
    },
    {
