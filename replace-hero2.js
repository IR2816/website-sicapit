...
function HeroSection() {
  return (
    <section id="beranda" className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden">
      ...
      {/* Content */}
      <div className="relative z-10 w-full max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center pt-24 md:pt-32">
        ...
        <motion.p initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.6 }} className="text-lg sm:text-xl text-slate-300 max-w-3xl mx-auto mb-12 leading-relaxed">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.7 }} className="flex flex-wrap items-center justify-center gap-3 mb-10">
          {[
            ...
            <span key={i} className="flex items-center gap-2 bg-surface/30 backdrop-blur-xl border border-white/10 text-white text-sm px-4 py-2.5 rounded-full hover:bg-surface/50 transition-colors drop-shadow-sm">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.9 }} className="flex flex-col sm:flex-row items-center justify-center gap-4">
          <Button size="lg" className="bg-brand text-white hover:bg-brand/90 px-8 py-6 text-lg rounded-full shadow-[0_0_40px_rgba(22,163,74,0.4)] transition-all font-bold" onClick={() => document.querySelector('#paket')?.scrollIntoView({ behavior: 'smooth' })}>
            Lihat Paket Rafting <ChevronRight className="w-5 h-5 ml-2" />
          </Button>
          <Button size="lg" variant="outline" className="border-white/20 text-white hover:bg-white/10 px-8 py-6 text-lg rounded-full backdrop-blur-sm font-bold bg-black/20" onClick={() => document.querySelector('#galeri')?.scrollIntoView({ behavior: 'smooth' })}>
            <Camera className="w-5 h-5 mr-2" /> Galeri Foto
          </Button>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 1.1 }} className="mt-20 grid grid-cols-2 md:grid-cols-4 gap-6 max-w-4xl mx-auto border-t border-white/10 pt-12 pb-16">
          {[
            ...
            <div key={i} className="text-center group">
              ...
              <div className="text-3xl sm:text-4xl font-heading font-extrabold text-white mb-1 drop-shadow-md">{stat.value}</div>
...
const parts = content.split('// ==================== HERO SECTION ====================');
const beforeHero = parts[0];
const textAfterFirst = parts[1];
const indexOfNextSection = textAfterFirst.indexOf('// ==================== ABOUT SECTION ====================');
const afterHero = textAfterFirst.substring(indexOfNextSection);
